        ...
        */
        ...
        for(int i=2; i<n; i++)
        {
            ...
            if(nums[i]>0)
            {
                ...
                int new_max_prod = max_prod * nums[i];
                if(new_max_prod > nums[i])
                {
                    curr_max_prod = new_max_prod;
                }
                int new_min_prod = min_prod * nums[i];
                if(new_min_prod < nums[i])
                {
                    curr_min_prod = new_min_prod;
                }
            ...
            else
            {
                ...
                int new_max_prod = nums[i] * min_prod;

                if(new_max_prod > nums[i])
                {
                    curr_max_prod = new_max_prod;
                }
                int new_min_prod = nums[i] * max_prod;
