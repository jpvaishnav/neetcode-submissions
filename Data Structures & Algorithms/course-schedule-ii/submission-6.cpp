...
we need to apply topological sort algo here, because topological sort algo picks node with 0 dependeny and update child dependencies
        ...
        - Use DFS with Kahn's algo
        - BFS optimizations:
            Replaced std::queue with Two Pointers: std::queue wraps around containers (like std::deque) which allocate memory blocks dynamically. 
            By treating your ans vector as the queue itself and tracking the front with a head index, you eliminate all queue push/pop memory allocation overhead.
            Vector Reservation: ans.reserve(numCourses) prevents the vector from constantly resizing, 
            reallocating memory, and copying elements across memory blocks while finding the order.
        */
