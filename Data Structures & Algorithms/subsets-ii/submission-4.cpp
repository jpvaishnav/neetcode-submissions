...
public:
    ...
    void backTrackWithState(vector<int>& nums,
                ...
                int n)
    {
        ...
        // every node is solution, as duplicate root node will be cut and wont be visited
        
        for(int i=start; i<n; i++)
        {
            // when visiting neighbor from top, have the cutting condition.
            // like [] is cutting second {1} in {1,1,2}
            if(i>start && nums[i]==nums[i-1])
            {
                // cutting duplicates, visualise with [1,1,2]
                continue;
            }
            //include ith element in solution
            solution.push_back(nums[i]);
            backTrackWithState(nums,ans,solution,i+1,n);
            solution.pop_back();

            // exclude ith element in solution
            //backTrack(nums,ans,solution,i+1,n);
            // not needed, as exclusion will be pushed at start of function
        }
    }
  ...
  nums[3]=2)   nums[3]=2)     nums[3]=2)        nums[3]=2)
        ...
        Cleaner solution: Similar to N Queen and Sudoko
            - where all states are explored in a for loop
            Yes, there is a cleaner and much more common backtracking solution that eliminates the boolean flag entirely.
            Instead of using a binary Include/Exclude decision tree, you use a Loop-Based State Exploration tree.
        ...
        */
        ...
        backTrackWithState(nums, ans, solution, 0, n);
        return ans;
    }
};
