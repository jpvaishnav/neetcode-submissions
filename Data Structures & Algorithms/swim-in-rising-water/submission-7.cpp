    ...
    int swimInWater(vector<vector<int>>& grid) {
        ...
        */
        ...
        - time complexity: O(N square) 
            - traversing each node exactly once(N)
            - inside that node, getting min using N
        N = n*n = total nodes
        - further optimization is possible through priority queue to find min distance node,
        and can be improved
        O(N*logN)
        */
        ...
        while(true)
        {
            ...
            for(int k=0; k<4; k++)
            {
                    ...
                    && finalised[adj_i][adj_j] == false)
                {
                    ...
                    // can we maintain min distance node here itsel, so that additional O(N square) lookup can be eliminated
