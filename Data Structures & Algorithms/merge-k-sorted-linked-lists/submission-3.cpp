...
public:
    ListNode* mergeKLists(vector<ListNode*>& lists) {
        ...
        **Pattern** - MergeSort algorithm where we merge two sorted arrays a and b to one sorted array c by using pointers
        ...
        */
        ...
        while(minHeap.empty() == false)
        {
            ...
            //optimized solution(based on Gemini recommendation): Above code block gives solution in 10 ms
            // this block is 5 ms. reason - above code is calling New() N times
            // -> Allocating Node memory and triggering heap allocation N times
            // -> which slows down code execution.
            // Instead of new node creation in memory, our final list can just reuse existing nodes and update pointers
            if(ans == NULL)
            {
                ans = top.second;
                ansHead = ans;
