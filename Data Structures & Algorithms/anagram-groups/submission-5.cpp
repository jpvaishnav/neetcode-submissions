...
public:
    ...
    string compute_hash(string str)
    {
        ...
        for(int i=0; i<26; i++)
        {
            ...
            // Use count[i] here for a, b, c, etc.
            // still taking 30 ms
            // syntax of a1b12c34 etc instead of #. I want to avoid printing 0 character frequencies in hash_key
            if(count[i] != 0)
            {
                hash_key += (char)('a' + i);
                hash_key += to_string(count[i]);
            }
            // this gives 20 ms - optimized solution
        }
        return hash_key;
    }
