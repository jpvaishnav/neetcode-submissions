...
public:
    ...
    int getIndexFromInOrder(vector<int> & inorder, int val)
    {
        ...
        // Inorder is sorted array for binary search tree(given question is only binary tree), so this logic can be reduced to O(logN) using binary search
        // In build binary_search gives true/false
        // Lower_bound gives index
        // auto it = lower_bound(inorder.begin(), inorder.end(), val);
        // cout<< "Element : "<< (*it) << "lower bound: "<< (it-inorder.begin()) << endl;
        // return (it-inorder.begin());

        // repeating same search logic again,
        // can we compute once and reuse next time -> HashTable
